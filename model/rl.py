...
class BertActorCritic(nn.Module):
    """ shared encoder between actor/critic"""
    def __init__(self, sent_encoder, art_encoder,
                 extractor, art_batcher, cuda, aux_device):
        super().__init__()
        self.aux_device = aux_device
        self.cuda = cuda

        self._sent_enc = sent_encoder.cuda() if cuda else sent_encoder
        self._art_enc = art_encoder.to(aux_device) if cuda else art_encoder
        self._ext = PtrExtractorRLStop(extractor).to(aux_device) if cuda else PtrExtractorRLStop(extractor)
        self._scr = PtrScorer(extractor).to(aux_device) if cuda else PtrScorer(extractor)
        self._batcher = art_batcher
    ...
    def forward(self, raw_article_sents, n_abs=None):
        ...
        # move to aux device
        if self.cuda:
            enc_sent = enc_sent.to(self.aux_device)
