...
from .attention import step_attention,augmented_attention
...
class Seq2SeqSumm(nn.Module):
    def __init__(self, vocab_size, emb_dim,
        ...
        #attention augment需要的东西
        self._attn_lstm = MultiLayerLSTMCells(2 * n_hidden, n_hidden, n_layer, dropout=dropout)
        self._linear_forget = nn.ModuleList([nn.Linear(n_hidden, n_hidden, bias=False) for _ in range(3)])
        self._linear_add = nn.ModuleList([nn.Linear(n_hidden, n_hidden, bias=False) for _ in range(3)])
        self._query_wq=nn.ModuleList([nn.Linear(n_hidden, n_hidden, bias=False) for _ in range(3)])
        ...
        self._decoder = AttentionalLSTMDecoder(
            ...
            self._attn_wq, self._projection,self._attn_lstm,self._linear_forget,self._linear_add, self._query_wq
        )
...
class AttentionalLSTMDecoder(object):
    def __init__(self, embedding, lstm, attn_w, projection,attn_lstm,linear_forget,linear_add,query_wq):
        super().__init__()
        self._embedding = embedding
        self._lstm = lstm
        self._attn_w = attn_w
        self._projection = projection
        self._attn_lstm = attn_lstm
        self._linear_forget = linear_forget
        self._linear_add = linear_add
        self._query_wq=query_wq
    def __call__(self, attention, init_states, target):
        ...
        for i in range(max_len):
            tok = target[:, i:i+1]
            logit, states, _ = self._step(tok, states, attention,key_arr)
            logits.append(logit)
        logit = torch.stack(logits, dim=1)
        return logit

    def _step(self, tok, states, attention,key_arr):
        ...
        context, score, key = augmented_attention(
            query, key_arr, attention, self._attn_lstm, states, self._linear_forget, self._linear_add,self._query_wq, attn_mask)
    ...
    def decode_step(self, tok, states, attention):
        key, mask = attention
        key_arr = key
        logit, states, score = self._step(tok, states, attention,key_arr)
