...
from decoding import Abstractor, RLExtractor, DecodeDataset, BeamAbstractor, BertRLExtractor
from decoding import make_html_safe


def decode(save_path, model_dir, split, batch_size,
           beam_size, diverse, max_len, cuda, args):
    start = time()
    # setup model
    ...
    ext_args = meta['net_args']['extractor']['net_args']
    if 'bert_type' in ext_args.keys():
        bert_type, tokenizer_cache = ext_args['bert_type'], ext_args['tokenizer_cache']
        bert_tokenizer = BertTokenizer.from_pretrained(bert_type, cache_dir=tokenizer_cache)

        extractor = BertRLExtractor(model_dir, bert_tokenizer, args.bert_max_sent_len, args.fix_bert,
                                    cuda=cuda, aux_device=aux_device)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path',  help='path to store/eval',default='/home/wanglihan/decoded4dotgg1/file')
    parser.add_argument('--model_dir', help='root of the full model',default='/home/wanglihan/save4dotgg/model')

    # dataset split
    data = parser.add_mutually_exclusive_group()
    data.add_argument('--val', action='store_true', help='use validation set',default=True)
    data.add_argument('--test', action='store_true', help='use test set', default=False)
    
    # bert option
    parser.add_argument('--bert_max_sent_len', type=int, action='store', default=500,
    ...
    decode(args.path, args.model_dir,
           ...
           args.max_dec_word, args.cuda, args)
