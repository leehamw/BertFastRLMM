...
@curry
def convert_batch_bert_extract_ptr(unk, word2id, batch):
    source_arts, extracts = map(list, unzip(batch))
    ext_word2id = dict(word2id)
    ...
    def convert_one(sample):
        source_sents, extracts = sample
        id_sents = conver2id(unk, ext_word2id, source_sents)
        return id_sents, extracts
        
    batch = list(map(convert_one, batch))
    ext_id2word = {i: w for w, i in ext_word2id.items()}
    return batch, ext_id2word

@curry
def convert_batch_bert_extract_ptr2(bert_tokenizer, batch):
    def convert_one(sample):
        source_sents, extracts = sample
        id_sents = [bert_tokenizer.encode(' '.join(words),
        add_special_tokens=True) for words in source_sents]
        return id_sents, extracts
    batch = list(map(convert_one, batch))
    return batch

@curry
def convert_batch_bert_extract_ptr3(bert_tokenizer, batch, max_len, max_sent):
    def convert_one(sample):
        source_sents, extracts = sample
        id_sents = [bert_tokenizer.encode(words, add_special_tokens=True) for words in source_sents]

        # trimmed sents
        trimmed = [i[: max_len - 1] + i[-1:] if len(i) > max_len else i for i in id_sents]
        
        # cleaned sents
        id_sents = trimmed[:max_sent]
        cleaned_extracts = list(filter(lambda e: e < len(id_sents), extracts))
        return id_sents, cleaned_extracts
...
@curry
def batchify_fn_bert_extract_ptr(pad, data, cuda=True):
    data, id2word = data
    source_lists, targets = tuple(map(list, unzip(data)))
    ...
    fw_args = (sources, src_nums, tar_in, id2word)
    loss_args = (target, )
    return fw_args, loss_args

@curry
def batchify_fn_bert_extract_ptr2(pad, data, cuda=True):
    source_lists, targets = tuple(map(list, unzip(data)))
