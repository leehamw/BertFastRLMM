...
def a2c_train_step(agent, abstractor, loader, opt, grad_fn, opt_step,
                   gamma=0.99, reward_fn=compute_rouge_l,
                   stop_reward_fn=compute_rouge_n(n=1), stop_coeff=1.0, cumul_step=1):
    indices = []
    probs = []
    baselines = []
    ext_sents = []
...
class A2CPipeline(BasicPipeline):
    ...
    def train_step(self, opt_step, cumul_step):
        # forward pass of model
        self._net.train()
        log_dict = a2c_train_step(
            self._net, self._abstractor,
            self._train_batcher,
            self._opt, self._grad_fn, opt_step,
            self._gamma, self._reward_fn,
            self._stop_reward_fn, self._stop_coeff, cumul_step
        )
        return log_dict
