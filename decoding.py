...
try:
    DATASET_DIR = '/home/wanglihan/CNN/finished_files/'
except KeyError:
...
class BertArticleBatcher(object):
    def __init__(self, tokenizer, max_len, max_sent, cuda=True):
        self._device = torch.device('cuda' if cuda else 'cpu')
        self._tokenizer = tokenizer
        self._bert_max_len = max_len
        self._bert_max_sent = max_sent
        self._pad = tokenizer.pad_token_id

    def __call__(self, raw_article_sents):
        articles = [self._tokenizer.encode(' '.join(words),
                add_special_tokens=True) for words in raw_article_sents]
        articles = self.trim_length(articles)
        article = pad_batch_tensorize(articles, self._pad, cuda=False).to(self._device)
        return article

    def trim_length(self, inputs):
        return [words[: self._bert_max_len - 1] + words[-1:] if len(words) > self._bert_max_len
        else words[:self._bert_max_len] for words in inputs][:self._bert_max_sent]
...
class BertRLExtractor(object):
    def __init__(self, ext_dir, tokenizer, max_len, fix_bert, cuda, aux_device):
        ext_meta = json.load(open(join(ext_dir, 'meta.json')))
        assert ext_meta['net'] == 'rnn-ext_abs_rl'
        ext_args = ext_meta['net_args']['extractor']['net_args']
        word2id = pkl.load(open(join(ext_dir, 'agent_vocab.pkl'), 'rb'))
        fix_bert = getattr(ext_args, 'fix_bert', fix_bert)
        ext_args['fix_bert'] = fix_bert
        ext_args['aux_device'] = aux_device
        extractor = BertPtrExtractSumm(**ext_args)
        agent = BertActorCritic(extractor._sent_enc,
                            extractor._art_enc,
                            extractor._extractor,
                            BertArticleBatcher(tokenizer, max_len, cuda),
                            cuda=cuda,
